...
void cli()
{
	...
	static int buffer_index = 0; // Indexing elements in cli_buffer
	...
	if (input == '_')
	{
		if (cmd_index > 0) // Reach the start of history
		{
			...
			buffer_index = strlen(cli_buffer);		  // Update buffer_index
		}
	}
	...
	else if (input != '\n')
	{
		...
		{
			uart_sendc(input);
			cli_buffer[buffer_index] = input; // Store into the buffer
			buffer_index++;
		}
		...
		else if (input == '\b')
		{
			...
			if (buffer_index >= 0)
			{
				cli_buffer[buffer_index] = '\0'; // Clear 1 char in buffer
				uart_puts("\033[1D\033[0K");	 // Clear 1 char in terminal (Move cursor left 1 character & Clear line from cursor right)
			}

			// Restart buffer_index if buffer is empty
	...
	else if (input == '\n')
	{
		...
		else
		{
			...
			cmd_index++;							  // Update index in command history
			cmd_history_length++;					  // Count number of commands in history
...
void show_prompt(int flag)
{
	if (flag == 0)
	{
		uart_puts("\033[0m");				 // Return to default
		uart_puts("\033[1;34mMoonveil\n> "); // Bold & Blue foreground text
		uart_puts("\033[0m");				 // Return to default
	}
	else
		uart_puts("Moonveil\n> ");
...
void feed(char *command, char *buffer)
{
	for (int i = 0; i < strlen(command); i++)
		buffer[i] = command[i];
	buffer[strlen(command)] = '\0'; // Enclose the string
}
